...
import { RegistryAuthProtocolTokenPayload, RegistryTokenCapability, CompositeAuthenticator } from "../src/auth";
...
describe("combined auth (jwt + username/password)", async () => {
  ...
  const basicAuth = (username: string, password: string) => ({
    Authorization: `Basic ${base64UrlEncode(`${username}:${password}`)}`,
  });
